            ...
            {jobs.length === 0 ? (
              ...
              jobs.map((job) => (
                <div 
                  ...
                  className="bg-white border border-[#dde9f3] rounded-[24px] p-5 md:p-6 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row md:items-center justify-between gap-6 border-l-4 border-l-[#3d7ab5] cursor-pointer group"
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                       ...
                       {role === 'employer' && (
                         <button 
                           onClick={(e) => { e.stopPropagation(); router.push(`/jobs/${job.id}/applications`); }} 
                           className="flex items-center gap-1.5 bg-[#eef5fb] text-[#3d7ab5] px-2 py-1 rounded-md border border-[#c8dff0] text-[10px] font-bold hover:bg-[#d9e8f5]"
                         >
                           <Users className="w-3 h-3" /> {t.applicants_btn}
                         </button>
                       )}
                    </div>
                    <h3 className="text-lg md:text-xl font-bold text-[#1a2533] mb-2 group-hover:text-[#3d7ab5] transition-colors">{job.title}</h3>
                       ...
                       <button 
                         ...
                         className={`px-6 md:px-8 py-3 rounded-xl font-bold text-xs transition-all ${
                           appliedJobIds.has(job.id) 
                             ...
                             : "bg-[#3d7ab5] text-white hover:bg-[#2c5f8a] hover:scale-105 active:scale-95"
                         }`}
                       >
